...
class MCPClient:
    ...
    def display_welcome_screen(self):
        ...
        for tool in self.tools:
            tool_descriptions.append(
                f"{self.TOOL_COLOR}🛠️  {tool.name} {Style.RESET_ALL}- {tool.description}"
            )

        tools_section = draw_section(
            tool_descriptions, self.box_width, self.MAIN_COLOR, emoji="🧰"
        )
        ...
        instructions_section = draw_section(
            instructions, self.box_width, self.MAIN_COLOR, emoji="ℹ️"
        )
    ...
    async def process_query(self, query: str) -> str:
        ...
        print(
            "\n"
            + draw_section(
                thinking_message, self.box_width, self.MAIN_COLOR, emoji="🔮"
            )
        )
        ...
        while True:
            ...
            for call_part, call in zip(new_parts, function_calls):
                ...
                print(
                    "\n"
                    + draw_section(
                        tool_message, self.box_width, self.TOOL_COLOR, emoji="🛠️"
                    )
                )
                ...
                # Show tool result
                result_message = (
                    f"Tool result: {json.dumps(function_response, indent=2)}"
                )
                print(
                    "\n"
                    + draw_section(
                        result_message, self.box_width, self.TOOL_COLOR, emoji="📦"
                    )
                )
    ...
    async def chat_loop(self):
        ...
        while True:
            ...
            input_section = draw_section(
                input_prompt, self.box_width, self.INPUT_COLOR, emoji="💬"
            )
            print("\n" + input_section)
            ...
            if query.lower() == "quit":
                ...
                goodbye_section = draw_section(
                    goodbye_message, self.box_width, self.MAIN_COLOR, emoji="👋"
                )
                print("\n" + goodbye_section + "\n")
                break
            ...
            response_section = draw_section(
                response, self.box_width, self.RESPONSE_COLOR, emoji="💡"
            )
            print("\n" + response_section)
...
async def main():
    ...
    try:
        ...
        loading_section = draw_section(
            loading_message, box_width, Fore.CYAN, emoji="🔄"
        )
        print("\n" + loading_section + "\n")
